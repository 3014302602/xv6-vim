...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     45a:	0f b6 80 f8 19 00 00 	movzbl 0x19f8(%eax),%eax
  ...
  for(i = 0; fmt[i]; i++){
    ...
    if(state == 0){
     ...
     596:	75 25                	jne    5bd <printf+0xff>
          s = "(null)";
     598:	c7 45 f4 d9 12 00 00 	movl   $0x12d9,-0xc(%ebp)
        while(*s != 0){
...
re_t re_compile(const char* pattern)
{
     ...
     950:	e9 55 02 00 00       	jmp    baa <re_compile+0x275>
  {
    ...
    switch (c)
     ...
     973:	8b 04 85 e0 12 00 00 	mov    0x12e0(,%eax,4),%eax
     97a:	ff e0                	jmp    *%eax
    {
      /* Meta-characters: */
...
/*    case '|': {    re_compiled[j].type = BRANCH;          } break; <-- not working properly */
     ...
     a0c:	8b 04 85 cc 13 00 00 	mov    0x13cc(,%eax,4),%eax
     a13:	ff e0                	jmp    *%eax
          {
            /* Meta-character: */
...
static int matchmetachar(char c, const char* str)
{
  ...
  switch (str[0])
     ...
     d81:	8b 04 85 9c 14 00 00 	mov    0x149c(,%eax,4),%eax
     d88:	ff e0                	jmp    *%eax
  {
...
static int matchone(regex_t p, char c)
{
  ...
  switch (p.type)
     ...
     efa:	8b 04 85 6c 15 00 00 	mov    0x156c(,%eax,4),%eax
     f01:	ff e0                	jmp    *%eax
  {
    case DOT:            return 1;
